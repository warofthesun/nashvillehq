...
<article <?php post_class(); ?> id="post-<?php the_ID(); ?>">
				...
				<div class="cover-color-overlay color-accent<?php echo esc_attr( $color_overlay_classes ); ?>"<?php echo $color_overlay_style; //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- We need to double check this, but for now, we want to pass PHPCS ;) ?>></div>
					...
					<header class="entry-header has-text-align-center">
						<div class="entry-header-inner section-inner medium">
							...
							<h1><?php the_title(); ?></h1>
							<?php the_content(); ?>
							<a href="/contact" class="button">Submit a resource</a>
							</div>
    ...
    $loop = new WP_Query( $args ); ?>
		<section id="resources" class="resources resources__grid alignwide">
			<h1>Resources & Opportunities</h1>
			<?php while ( $loop->have_posts() ) : $loop->the_post(); ?>
				<?php get_template_part( 'template-parts/resource-item' ); ?>
			<?php endwhile; ?>
		</section>
    <?php wp_reset_postdata(); ?>
		...
		<p style="text-align:center;">
			<a href="/resource" class="button" style="width: 250px;">View All</a>
		</p>
		<?php
		wp_link_pages(
			array(
